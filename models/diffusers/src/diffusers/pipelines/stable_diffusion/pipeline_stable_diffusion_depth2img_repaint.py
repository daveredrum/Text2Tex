...
class StableDiffusionDepth2ImgRepaintPipeline(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        depth_map: torch.FloatTensor = None,
        strength: float = 0.8,
        ...
        # 5 Prepare image mask
        image_mask, _ = prepare_mask_and_masked_image(image, mask_image)
        ...
        # 7. Prepare latent variables and repaint mask
        latents = self.prepare_latents(
            image, latent_timestep, batch_size, num_images_per_prompt, text_embeddings.dtype, device, generator
        )
        latents_mask = transforms.Resize(latents.shape[2], interpolation=transforms.InterpolationMode.NEAREST)(image_mask)
        if str(latents.dtype) == "torch.float16": latents_mask = latents_mask.type(torch.HalfTensor)
        latents_mask = latents_mask.to(device)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # crop known parts of the latents
                noise_latents = self.prepare_latents(
                    image, t-1, batch_size, num_images_per_prompt, text_embeddings.dtype, device, generator
                )
                latents_known_mask = 1 - latents_mask
                known_latents = noise_latents * latents_known_mask
                
                # crop unknown parts of the latents
                latents_unknown_mask = latents_mask
                unknown_latents = latents * latents_unknown_mask

                # combine known and unknown latents
                latents = known_latents + unknown_latents
